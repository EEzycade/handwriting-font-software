...
def process():
    ...
    templateType = secure_filename(request.form.get(
    ...
    if os.path.exists(os.path.join(app.config["FONTS_FOLDER3"], base_font)):
        ...
        flash(
            f'The base font \'{request.form["base_font"]}\' does not exist. Using the default base font \'{base_font}\' instead.', 'warning')
        print(
            f'The base font \'{request.form["base_font"]}\' does not exist. Using the default base font \'{base_font}\' instead.')
    ...
    for cutImage, symbol in zip(cut_images, flattened_template):
        if symbol != None:
            if cutImage.size == 0:
                ...
                imwrite(os.path.join(
                    cut_image_path, f"{symbol}{1 if symbol.isupper() else 2}.jpg"), cutImage)
                imwrite(os.path.join(
                    unboxed_image_path, f"{symbol}{1 if symbol.isupper() else 2}.jpg"), bmark.image.unbox(cutImage))
    ...
    font_path = os.path.join(
        app.config['FONTS_FOLDER2'], internal_name + ".otf")
    clean(svg_path)
...
def create_template(template_name):
    ...
    image.save(os.path.join(
        "app", app.config["TEMPLATE_IMAGES_FOLDER"], template_name + ".png"))
